# frozen_string_literal: true

module Carbon
  class ToastNotificationComponentPreview < ViewComponent::Preview
    # @param kind select { choices: [info, success, warning, error, info_square] }
    # @param title text
    # @param subtitle text
    # @param caption text
    # @param hide_close_button toggle
    # @param low_contrast toggle
    # @param timeout number
    def default(
      kind: :info,
      title: 'Notification title',
      subtitle: 'Subtitle text',
      caption: '00:00:00 AM',
      hide_close_button: false,
      low_contrast: false,
      timeout: 0
    )
      render Carbon::ToastNotificationComponent.new(
        kind: kind.to_sym,
        title: title,
        subtitle: subtitle,
        caption: caption,
        hide_close_button: hide_close_button,
        low_contrast: low_contrast,
        timeout: timeout.positive? ? timeout : nil
      )
    end

    # @!group All Kinds

    def info
      render Carbon::ToastNotificationComponent.new(
        kind: :info,
        title: 'Info notification',
        subtitle: 'This is an informational message'
      )
    end

    def success
      render Carbon::ToastNotificationComponent.new(
        kind: :success,
        title: 'Success!',
        subtitle: 'Your changes have been saved'
      )
    end

    def warning
      render Carbon::ToastNotificationComponent.new(
        kind: :warning,
        title: 'Warning',
        subtitle: 'Please review your input'
      )
    end

    def error
      render Carbon::ToastNotificationComponent.new(
        kind: :error,
        title: 'Error',
        subtitle: 'Something went wrong'
      )
    end

    def info_square
      render Carbon::ToastNotificationComponent.new(
        kind: :info_square,
        title: 'Info square',
        subtitle: 'Alternative info style'
      )
    end

    # @!endgroup

    def with_caption
      render Carbon::ToastNotificationComponent.new(
        title: 'Order delivered',
        subtitle: 'Your package has arrived',
        caption: '10:30 AM'
      )
    end

    def auto_dismiss
      render Carbon::ToastNotificationComponent.new(
        title: 'Auto-dismiss',
        subtitle: 'This notification will disappear in 3 seconds',
        timeout: 3000
      )
    end

    def low_contrast
      render Carbon::ToastNotificationComponent.new(
        title: 'Low contrast',
        subtitle: 'Low contrast notification',
        low_contrast: true
      )
    end

    def without_close_button
      render Carbon::ToastNotificationComponent.new(
        title: 'No close button',
        subtitle: 'This notification cannot be dismissed',
        hide_close_button: true
      )
    end
  end
end
